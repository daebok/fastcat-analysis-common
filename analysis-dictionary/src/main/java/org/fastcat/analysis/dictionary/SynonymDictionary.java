...
public class SynonymDictionary extends MapDictionary {
	...
	public Set<CharSlice> getWordSet() {
		return wordSet;
	}
	
	public void setWordSet(Set<CharSlice> wordSet) {
		this.wordSet = wordSet;
	}
	
	public Set<CharSlice> getUnmodifiableWordSet() {
	...
	public void addEntry(String keyword, Object[] values) {
		...
		ArrayList<CharSlice> list = new ArrayList<CharSlice>(4);

		CharSlice mainWord = null;
		if (keyword != null) {
			keyword = keyword.trim();
			if (keyword.length() > 0) {
				mainWord = new CharSlice(keyword);
		...
		for (int k = 0; k < synonyms.length; k++) {
			...
			if (synonym.length() > 0) {
				CharSlice word = new CharSlice(synonym);
				list.add(word);
				wordSet.add(word);
			}
		}
		...
		if (mainWord == null) {
			...
			for (int j = 0; j < list.size(); j++) {
				CharSlice key = list.get(j);
				CharSlice[] value = new CharSlice[list.size() - 1];
				int idx = 0;
				for (int k = 0; k < list.size(); k++) {
					CharSlice val = list.get(k);
					if (!key.equals(val)) {
						// 다른것만 value로 넣는다.
						value[idx++] = val;
					}
			...
			// 단방향.
			CharSlice[] value = new CharSlice[list.size()];
			int idx = 0;
			for (int j = 0; j < value.length; j++) {
				CharSlice word = list.get(j);
			...
			if (value.length > 0) {
				CharSlice[] value2 = map.get(mainWord);
				if (value2 != null) {
					// 이전값과 머징.
					value = mergeSynonyms(value2, value);
				}
	...
	private CharSlice[] mergeSynonyms(CharSlice[] value2, CharSlice[] value) {
		int removedCount = 0;
		...
		CharSlice[] list = new CharSlice[newSize];
		int i = 0;
		for(CharSlice v : value2){
			if(v != null){
				list[i++] = v;
			}
		}
		for(CharSlice v : value){
			if(v != null){
				list[i++] = v;
			}
		}
	...
	private String join(CharSlice[] list) {
		String result = "";
		for (int i = 0; i < list.length; i++) {
	...
	public void writeTo(OutputStream out) throws IOException {
		...
		// write synonyms
		Iterator<CharSlice> synonymIter = wordSet.iterator();
		for (; synonymIter.hasNext();) {
			CharSlice value = synonymIter.next();
	...
	public void readFrom(InputStream in) throws IOException {
		...
		for (int entryInx = 0; entryInx < size; entryInx++) {
			wordSet.add(new CharSlice(input.readUString()));
		}
	}
